...
Программа представляет собой фильтр DNS трафика, работающий через механизм NFQUEUE в Linux. Фильтр перехватывает DNS-пакеты, извлекает поля DNS-запроса/ответа, сопоставляет их с правилами из конфигурационного файла и принимает решение: пропустить или заблокировать пакет.

Поддерживаемые поля DNS для фильтрации:
- `qname` - доменное имя (поддерживаются регулярные выражения)
...
- `qdcount` — количество вопросов в DNS-запросе
- `ancount` — количество ответов в DNS-пакете
