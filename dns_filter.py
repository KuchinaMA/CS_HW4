...
class DNSFilter:
    ...
    def get_query_type_description(self, qtype):
        types = {v: k for k, v in DNS_TYPES.items()}
    ...
    def check_rule(self, rule, dns_data):
        ...
        if field == 'qtype' and isinstance(value, str):
            value = DNS_TYPES.get(value.upper(), None)
        ...
        try:
            if operator == '==':
                ...
                return re.search(value, str(field_value)) is not None
        except:
            return False
        return False
...
def main():
    ...
    dns_filter = DNSFilter(args.rules)
    queue = netfilterqueue.NetfilterQueue()
    
    try:
        queue.bind(args.queue_num, dns_filter.process_packet)
